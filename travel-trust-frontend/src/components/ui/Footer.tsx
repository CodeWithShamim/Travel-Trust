    ...
    >
      <div className="max-w-[1200px] mx-auto pt-10">
        <Row className="flex !gap-4 flex-col !md:flex-row !w-full">
          {items.map((item, index) => (
            <Col key={index} xs={24} lg={7} className="!flex">
              <div className="bg-[#FFD20A] text-white w-8 md:w-10 lg:w-12 h-6 md:h-10 lg:h-12 justify-center items-center">
                {item.icon}
              </div>
        ...
        <div className="text-left">
          ©2025 TravelTrust. All rights reserved. Powered by{' '}
          <span className="text-[#FFD20A] text-lg">Zama</span> Protocol
        </div>
      </div>
    </div>
  );
